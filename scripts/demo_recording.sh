...
demo_main() {
    ...
    # Ensure the pipeline-saved kernel has the marker for the demo
    if ! grep -q "saved for manual hipcc" ported_kernels/nvidia_shfl_scan.hip.cpp 2>/dev/null; then
        local f=ported_kernels/nvidia_shfl_scan.hip.cpp
        if [[ -f "$f" ]]; then
            sed -i '1i// pipeline: Not compiled — saved for manual hipcc' "$f"
            echo "  ▸ (re-added marker for demo purposes)"
        fi
    fi

    # ACTUALLY run the firecrawl agent — real terminal output
    set +e
    bash scripts/firecrawl_auto_compile.sh 2>&1 | sed 's/^/  │ /'
    firecrawl_exit=$?
    set -e
    echo ""
    if [[ $firecrawl_exit -eq 0 ]]; then
        echo "  ✓ Firecrawl agent completed"
    else
        echo "  ⚠ Firecrawl agent exit code: $firecrawl_exit"
    fi
    pause 3

    # ── PHASE 5: Show the generated proof harness ───────────
    hr "PHASE 5: Auto-Generated Proof Harness"
    echo "  The firecrawl agent extracted the kernel body and generated"
    echo "  a self-contained test harness from the flagged file."
    echo ""
    # Show whatever harness was generated
    if [[ -f /tmp/nvidia_shfl_scan_proof.hip.cpp ]]; then
        echo "  (Generated by firecrawl agent — showing harness)"
        wc -l /tmp/nvidia_shfl_scan_proof.hip.cpp | sed 's/^/  │ /'
        echo ""
        cat /tmp/nvidia_shfl_scan_proof.hip.cpp | sed 's/^/  │ /'
    else
        # Fallback: show the known working proof
        echo "  (Using reference proof file — same structure as agent generates)"
        echo ""
